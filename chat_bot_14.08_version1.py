...
def portfolio_walkforward_backtest(
    ...
    for seg_start in range(start_i, len(all_idx) - 1, step):
        ...
        if rebalance.upper().startswith("W"):
            ...
            rebals = pd.date_range(test_idx[0], test_idx[-1], freq="M")
        ...
        for d in test_idx:
            ...
            for t in list(held.keys()):
                ...
                try:
                    ...
                    trend_keep_ok = True
                try:
                    mom_keep_ok = float(row["mom126"]) >= 0.0
                except Exception:
                    mom_keep_ok = True
    ...
    if len(daily_rets) > 1:
        ...
        cagr = 0.0; dd = 0.0; sharpe = 0.0; turn_yr = 0.0

    return {
        "oos_trades": trades,
        "oos_equity": equity,
        "oos_CAGR": float(cagr),
        "oos_maxDD": float(abs(dd)),
...
# ====== PATCH: robust walk-forward backtest (risk-parity + trend/momentum/low-vol, monthly/weekly) ======
# This overrides the earlier definition (Python will use the latest one).
from dataclasses import dataclass, field

def _ann_return(daily: pd.Series) -> float:
    if daily.empty: return 0.0
    return (1 + daily.mean())**252 - 1

def _sharpe(daily: pd.Series) -> float:
    if daily.std() == 0 or np.isnan(daily.std()): return 0.0
...
def _turnover(weights: pd.DataFrame) -> float:
    if weights is None or weights.empty: return 0.0
    w = weights.fillna(0.0).copy()
    delta = w.diff().abs().sum(axis=1).dropna()
    if delta.empty: return 0.0
    # infer rebalance frequency
    d = w.index.to_series().diff().dropna().dt.days
    per_year = max(1, int(round(365/ (d.mean() if len(d) else 30))))
    return float(delta.mean()*per_year)

def _resample_index(prices: pd.DataFrame, rule: str) -> pd.DatetimeIndex:
    # Return valid dates in price index closest to resample rule boundaries
    if rule.upper().startswith("W"):  # weekly
        idx = prices.resample(rule).last().index
    else:
        idx = prices.resample("M").last().index
    return idx[idx.isin(prices.index)]
...
def _scores(pr: pd.DataFrame, mom_lb: int, trend_ma: int, vol_lb: int) -> pd.DataFrame:
    rets = pr.pct_change()
    mom = (pr / pr.shift(mom_lb) - 1.0).clip(-1, 2)
    trend = (pr > pr.rolling(trend_ma).mean()).astype(float)*2-1
    vol = rets.rolling(vol_lb).std()
    # normalize
    def z(df): return (df-df.mean())/(df.std()+1e-9)
    score = 0.5*z(mom) + 0.3*trend + 0.2*z(-vol)
    return score

def _risk_parity(pr_window: pd.DataFrame, vol_lb: int) -> pd.Series:
    vol = pr_window.pct_change().rolling(vol_lb).std().iloc[-1].replace(0, np.nan)
    inv = 1.0/vol
    inv = inv.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    if inv.sum() == 0: return pd.Series(0, index=pr_window.columns)
    return inv/inv.sum()

def _vol_scale(daily: pd.Series, target_ann: float) -> float:
    if len(daily) < 20: return 1.0
    realized = daily.std()*np.sqrt(252)
    if realized == 0 or np.isnan(realized): return 1.0
    return float(np.clip(target_ann/realized, 0.0, 1.0))
...
def portfolio_walkforward_backtest(
    ...
    # Clean tickers and ensure unique
    tickers = sorted(set([t for t in tickers if isinstance(t, str) and t.strip()]))

    # Add a cash-like fallback if available
    cash_candidates = ["SHY","BIL","SGOV","GBIL"]
    cash = next((c for c in cash_candidates if c in tickers), None)
    if cash is None: tickers = tickers + ["SHY"]; cash = "SHY"
    ...
    cfg = _WeightsCfg(top_k=top_k, rebalance=rebalance, cost_bps=cost_bps, slip_bps=slip_bps)

    # Walk-forward
    oos_daily = []
    all_idx = prices.index
    # translate months to index steps ~22 trading days per month
    step = int(test_m*22)
    warm = int(train_m*22)
    if len(all_idx) < warm + step + 5:
        return {"oos_trades": 0, "oos_equity": 1.0, "oos_CAGR": 0.0, "oos_maxDD": 0.0, "oos_turnover": 0.0, "oos_sharpe": 0.0}

    weights_records = []  # for turnover
    i = warm
    while i + step < len(all_idx):
        end_idx = i + step
        window_idx = all_idx[:end_idx]
        pr_win = prices.loc[window_idx]

        # compute scores on full history to current point; select top-K in universe excluding cash
        score = _scores(pr_win.drop(columns=[cash], errors="ignore"), cfg.mom_lb, cfg.trend_ma, cfg.vol_lb)
        # determine rebalance dates inside [i, end_idx)
        rb = _resample_index(pr_win, cfg.rebalance)
        rb = rb[(rb >= all_idx[i]) & (rb < all_idx[end_idx])]
        if len(rb) == 0: 
            i = end_idx
            continue

        prev_w = pd.Series(0.0, index=prices.columns)
        port_daily = pd.Series(0.0, index=pr_win.index[pr_win.index.get_loc(rb[0]): all_idx[end_idx]])
        for d in rb:
            pool = [t for t in score.columns if t != cash and t in pr_win.columns]
            sc = score.loc[d, pool].sort_values(ascending=False)
            picks = list(sc.iloc[:cfg.top_k].index) if len(sc) else []
            if len(picks) == 0:
                w = pd.Series(0.0, index=prices.columns)
                w[cash] = 1.0
            else:
                rp = _risk_parity(pr_win[picks], cfg.vol_lb)
                if rp.sum() == 0 or rp.isna().all():
                    rp = pd.Series(1/len(picks), index=picks)
                w = pd.Series(0.0, index=prices.columns)
                w.loc[picks] = rp.values
                # fill remainder with cash
                w[cash] = max(0.0, 1.0 - w.sum())

            # daily returns until next rebalance (or segment end)
            # cost on rebalance proportional to weight change
            turnover = float((w - prev_w).abs().sum())
            cost = turnover * (cfg.cost_bps + cfg.slip_bps) / 10000.0

            # compute daily portfolio returns on [d, next_d)
            if d == rb[-1]:
                next_d = all_idx[end_idx]
            else:
                next_d = rb[rb.get_loc(d)+1]
            win = pr_win.loc[d:next_d]
            rets = win.pct_change().fillna(0.0)
            prt = (rets * w).sum(axis=1)
            if len(prt) > 0:
                prt.iloc[0] = prt.iloc[0] - cost
                port_daily.loc[prt.index] = prt.values
            prev_w = w
            weights_records.append((d, w))

        # vol targeting scale
        scale = _vol_scale(port_daily, cfg.target_vol)
        oos_daily.append(port_daily * scale)
    ...
    mets = {
        ...
        "oos_maxDD": float(_maxdd(cum)),
        "oos_sharpe": float(_sharpe(oos)),
